            ...
            <div class="flex-1">
                <div class="bg-white rounded-xl shadow-sm overflow-hidden cart-items"
                    ...
                    @if ($cartItems->isEmpty())
                        <p class="p-4">Keranjang Kosong.</p>
                    @else
                            ...
                            <tbody>
                                @foreach ($cartItems as $item)
                                    <tr>
                                        ...
                                        <td class="border p-2">Rp {{ number_format($item->produk->harga, 0, ',', '.') }}</td>
                                        <td class="border p-2">
                                            <button wire:click="updateQuantity({{ $item->id }}, 'decrease')"
                                                class="px-2 bg-gray-300">-</button>
                                            {{ $item->quantity }}
                                            <button wire:click="updateQuantity({{ $item->id }}, 'increase')"
                                                class="px-2 bg-gray-300">+</button>
                                        </td>
                                        <td class="border p-2">Rp {{ number_format($item->produk->harga * $item->quantity, 0, ',', '.') }}</td>
                                        <td class="border p-2">
